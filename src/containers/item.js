...
class Item extends Component {
                    ...
                    <Content className="container">
                        <Descriptions title={this.getWord("order-details")}>
                            <Descriptions.Item label={this.getWord("order")}>{
                                    order_customer && <Link to={`/o/${order_customer.order_id}`}>
                            ...
                            <Descriptions.Item label={this.getWord("customer-name")}>{order_customer.customer_name}</Descriptions.Item>
                            ...
                            <Descriptions.Item label={this.getWord("date")}>{
                                    this.state.language === "vietnamese"
                                        ? order_customer.order_date_viet
                            ...
                            <Descriptions.Item label={this.getWord("hotel")}>{order_customer.hotel}</Descriptions.Item>
                            <Descriptions.Item label={this.getWord("room")}>{order_customer.room}</Descriptions.Item>
                                ...
                                <TabPane
                                    ...
                                    <Row gutter={40}>
                                        <Col span={8}>
                                            ...
                                            <Divider />                                            <Descriptions title={this.getWord("notes")} column={1}>
                                    ...
                                    details.notes
                                        ...
                                        <Col span={8}>
                                            <Descriptions title={this.getWord("fabric")} column={1} bordered={false}>
                                                {
                                                    fabrics.length > 0 && fabrics.map(
                                                        ...
                                                        />} ><Meta
                                                        title={<Link to={`/f/${o.unique_code}`}>
                                                        {o.unique_code}
                                                    </Link>}

                                                        description={o.notes
                                                            ? o.notes
                                                            : this.getWord("no-notes")}/></Card>
                                                        </Descriptions.Item>
                                                    )
                                                }
                                            </Descriptions>
                                            ...
                                            fittings.length > 0
                                                ? (fittings.map(f => (
                                                    <Timeline.Item
                                                        ...
                                                        {
                                                            f.changes_needed !== "" && <Row>{this.getWord("changes-needed")}:
                                                                    <b>{f.changes_needed}</b>
                                                                </Row>
                                                        }
                                                        <Row>{this.getWord("status")}:
                                                            <b>{
                                                                    this.state.language === "vietnamese"
    ...
    getMeasurements = id => {
        fetch(
            api + "web_order_measurement_items?filter[where][item_id]=" + id
        )
            .then(res => {
                if (res.ok) {
                    return res.json();
                ...
                measurements
                    ? this.setState({
                        measurements: measurements[0]
                            ? _.sortBy(measurements[0].measurements, [function (o) {
                                    return o.order;
                                }
                            ])
                            : []
    ...
    getItemFittings = id => {
        fetch(
            api + "web_order_fittings?filter[where][item_id]=" + id
        )
            .then(res => {
                if (res.ok) {
                    return res.json();
    ...
    getItemFabrics = id => {
        fetch(
            api + "web_order_fabrics?filter[where][item_id]=" + id
        )
            .then(res => {
                if (res.ok) {
                    return res.json();
    ...
    getItemOptions = id => {
        fetch(
            api + "web_order_options?filter[where][item_id]=" + id
        )
            .then(res => {
                if (res.ok) {
                    return res.json();
    ...
    getItemImages = id => {
        fetch(
            api + "web_order_images?filter[where][item_id]=" + id
        )
            .then(res => {
                if (res.ok) {
                    return res.json();
    ...
    getItemInfo = id => {
        fetch(
            api + "web_order_items?filter[where][item_id]=" + id
        )
            .then(res => {
                if (res.ok) {
                    return res.json();
            ...
            .then(details => {
                ...
                this.setState({error: false});
                   ...
                   switch(details[0].status_id){
                        case "done": this.setState({status_id:2}); break;
                        case "pending": this.setState({status_id:-1}); break;
                        default:this.setState({status_id:1})
                    }
